...
bit_mining() {
  ...
  while [  $COUNTER -lt $round_num ]; do
    ...
    do
      port_rpc=$(( $RPC_PORT + $j ))
      TR=`${BIT_ROOT}/src/ybc -regtest -rpcuser=ybtcrpc -rpcpassword=P0 -rpcconnect=localhost:${port_rpc} generate 1`
      sleep 1
...
bit_mining_one() {
  ...
  TR=`${BIT_ROOT}/src/ybc -regtest -rpcuser=ybtcrpc -rpcpassword=P0 -rpcconnect=localhost:${port_rpc} generate ${round_num}`
  sleep 1
