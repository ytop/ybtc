...
#ifndef FABCOIN_SCEXECUTIVE_HPP
#define FABCOIN_SCEXECUTIVE_HPP
...
struct ExecutiveWarnChannel : public LogChannel {
    static const char* name();
...
public:
    ...
    OnOpFunc onOp()
    {
        return [=](uint64_t _steps, uint64_t _PC, Instruction _inst, bigint _newMemSize, bigint _gasCost, bigint _gas, VM* _vm, ExtVMFace const* _extVM) { (*this)(_steps, _PC, _inst, _newMemSize, _gasCost, _gas, _vm, _extVM); };
    }
...
public:
    ...
    Executive(State& _s, unsigned _level = 0) : m_s(_s), m_depth(_level) {}
	 ...
	 */
    Executive(State& _s, unsigned _txIndex, unsigned _level = 0);

    Executive(Executive const&) = delete;
    void operator=(Executive) = delete;

    /// Initializes the executive for evaluating a transaction. You must call finalize() at some point following this.
    void initialize(bytesConstRef _transaction) { initialize(Transaction(_transaction)); }
    ...
    bool create(Address _txSender, u256 _endowment, u256 _gasPrice, u256 _gas, bytesConstRef _code, Address _originAddress, Address _contractAddress);
