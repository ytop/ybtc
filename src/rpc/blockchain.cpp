...
{
    double dDiff = (double)0x0000ffff ;
...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("stateroot", blockindex->hashStateRoot.GetHex()));
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.push_back(Pair("mediantime", (int64_t)blockindex->GetMedianTimePast()));
    result.push_back(Pair("height", (uint64_t)block.nHeight));
    result.push_back(Pair("chainwork", blockindex->nChainWork.GetHex()));
...
UniValue getblockheader(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"stateroot\" : \"xxxx\", (string) The state root\n"
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of ybtc addresses\n"
