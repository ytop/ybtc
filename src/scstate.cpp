...
const char* StateSafeExceptions::name() { return ""; }
const char* StateDetail::name() { return ""; }
const char* StateTrace::name() { return ""; }
const char* StateChat::name() { return ""; }

State::State(u256 const& _accountStartNonce, OverlayDB const& _db, BaseState _bs) : m_db(_db),
                                                                                    m_state(&m_db),
...
State::State(State const& _s) : m_db(_s.m_db),
...
OverlayDB State::openDB(std::string const& _basePath, h256 const& _genesisHash, WithExisting _we)
{
    ...
    path += "/" + toHex(_genesisHash.ref().cropped(0, 4)) + "/" + std::to_string(c_databaseVersion);
    ...
    if (!status.ok() || !db) {
        if (boost::filesystem::space(path + "/state").available < 1024) {
            ...
            BOOST_THROW_EXCEPTION(std::range_error("Not Enough Available Space"));
        } else {
            cwarn << status.ToString();
            cwarn << "Database " << (path + "/state") << "already open. You appear to have another instance of ethereum running. Bailing.";
            BOOST_THROW_EXCEPTION(std::range_error("Database Already Open"));
        }
    }
...
u256 const& State::requireAccountStartNonce() const
{
    if (m_accountStartNonce == Invalid256)
        BOOST_THROW_EXCEPTION(std::range_error("Invalid Account Start Nonce State"));
...
void State::noteAccountStartNonce(u256 const& _actual)
{
    if (m_accountStartNonce == Invalid256)
        ...
        BOOST_THROW_EXCEPTION(std::range_error("Incorrect Account Start Nonce In State"));
}
...
void State::commit(CommitBehaviour _commitBehaviour)
{
    if (_commitBehaviour == CommitBehaviour::RemoveEmptyAccounts)
        removeEmptyAccounts();
    auto c = sc::commit(m_cache, m_state);
    m_touched.insert(c.begin(), c.end());
    m_changeLog.clear();
    m_cache.clear();
...
{
    std::unordered_map<Address, u256> ret;
    for (auto& i : m_cache)
        if (i.second.isAlive())
...
ExecutionResult State::execute(Transaction const& _t, Permanence _p, OnOpFunc const& _onOp)
{
    auto onOp = _onOp;
#if ETH_VMTRACE
        ...
        if (cache && !cache->isAlive())
            ...
            if ((cache && cache->codeHash() == EmptySHA3) || (!cache && r && (h256)r[3] != EmptySHA3)) {
                ...
                for (auto const& j : mem)
                    if (j.second)
                        contout << std::endl
                                << (delta.count(j.first) ? back.count(j.first) ? " *     " : " +     " : cached.count(j.first) ? " .     " : "       ") << std::hex << std::nouppercase << std::setw(64) << j.first << ": " << std::setw(0) << j.second;
                    else
                        contout << std::endl
                                << "XXX    " << std::hex << std::nouppercase << std::setw(64) << j.first << "";
            } else
                contout << " [SIMPLE]";
