...
#ifndef FABCOIN_SCRLP_HPP
#define FABCOIN_SCRLP_HPP
...
template <class T>
struct Converter {
    static T convert(RLP const&, int) { BOOST_THROW_EXCEPTION(std::range_error("Bad Cast")); }
};
...
public:
    ...
    {
        if (!isList()) BOOST_THROW_EXCEPTION(std::range_error("Bad Cast"));
        return items();
    }
    ...
    {
        if (!isData()) BOOST_THROW_EXCEPTION(std::range_error("Bad Cast"));
        return length();
    }
    ...
    {
        auto l = length();
        if (l > m_data.size()) BOOST_THROW_EXCEPTION(std::range_error("Bad RLP"));
...
template <class T>
inline T RLP::convert(int _flags) const
{
...
public:
    ...
    {
        if (!m_listStack.empty()) BOOST_THROW_EXCEPTION(std::range_error("listStack is not empty"));
        return m_out;
    }
