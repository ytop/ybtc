...
#if defined(NDEBUG)
#error "Ybtc cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Ybtc Signed Message:\n";

std::unique_ptr<sc::State> pState; //fasc
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (!CheckCasino(block.GetHash(), block.nBits, consensusParams))
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Force block reward to zero in 60 years (20*3)
    if (halvings >= 20)
        return 0;
    ...
    // Subsidy is cut in half every 9,500,000 blocks which will occur approximately every 3 years.
    //   Generate 1 block every 10 seconds,
    //      9460800 = 3600*24*365*3/10
    //   Total coins : 150,000,000
    //      151372800 = 9460800 * 8 * 2
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
static DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (pindex->pprev->hashStateRoot == uint256()) {
        pState->setRoot(sc::sha3(sc::rlp("")));
    } else {
        pState->setRoot(sc::uintToh256(pindex->pprev->hashStateRoot));
    }
...
static bool CheckContractRefundFee(const CBlock& block, std::vector<CTxOut>& vRefundGasFee)
{
    ...
    if (voutSize < vRefundSize + 2) {
        return false;
    }
    for (size_t i = 0; i < vRefundSize; i++) {
        if (vtxCoinBase.vout[voutSize - 2 - i] != vRefundGasFee[vRefundSize - 1 - i]) {
            return false;
        }
    }
    return true;
...
bool CheckContractTx(const CBlock& block, CValidationState& state, bool hasContract)
{
    ...
    if (!smct.GetBlockContract(checkBlock, vRefundGasFee)) {
        pState->setRoot(oldHashStateRoot);
        LogPrintf("Execute contract failed...\n");
        return state.DoS(100, false, REJECT_INVALID, "bad-contract when connecting block", true, "get contract failed");
    }

    if (!CheckContractRefundFee(block, vRefundGasFee)) {
        pState->setRoot(oldHashStateRoot);
        LogPrintf("Refund fee not correct...\n");
        return state.DoS(100, false, REJECT_INVALID, "bad-contract-refund when connecting block", true, "contract refund fee mismatched");
    }
    

    if (block.hashStateRoot != uint256(h256Touint(sc::h256(pState->rootHash())))) {
        pState->setRoot(oldHashStateRoot);
        LogPrintf("hashStateRoot mismatch...\n");
        return state.DoS(100, false, REJECT_INVALID, "bad-txnstateroot when connecting block", true, "hashStateRoot mismatch");
    }

    
    if (checkBlock.GetHash() != block.GetHash()) {
        pState->setRoot(oldHashStateRoot);
        LogPrintf("hash mismatch...\n");
        return state.DoS(100, false, REJECT_INVALID, "bad-proofhash when connecting block", true, "hashproof mismatch");
    }
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.HasCreateOrCall()) hasContract = true;
    ...
    if (!CheckContractTx(block, state, hasContract))
        return state.DoS(100, error("%s: CheckContractTx failed", __func__), REJECT_INVALID, "block-validation-failed");
    ...
    if (fJustCheck) {
        sc::h256 prevHashStateRoot(sc::sha3(sc::rlp("")));

        if (pindex->pprev->hashStateRoot != uint256()) {
            prevHashStateRoot = sc::uintToh256(pindex->pprev->hashStateRoot);
        }
        pState->setRoot(prevHashStateRoot);
        return true;
    }
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW && !CheckCasino(block.GetHash(), block.nBits, consensusParams))
...
bool CallContract(sc::Address scAddr, std::vector<unsigned char> opcode, std::vector<unsigned char>* output, bool fStateChange)
{
    sc::h256 oldHashStateRoot(pState->rootHash());
    try {
        sc::Transaction scTx;
...
// =================== CASINO ===========================
/*
bool ExistGenesisContract()
{
    //return pState->addressInUse(sc::h160(ToByteVector(GENESIS_CONTRACT_ADDRESS_ETH)));
    return pState->addressInUse(sc::h160(ParseHex(GENESIS_CONTRACT_ADDRESS_ETH)));
}
*/
/*
bool CasinoContract()
{
    sc::Transaction scTx(true, sc::u256(), sc::u256(DEFAULT_GAS_PRICE), sc::u256(DEFAULT_GAS_LIMIT_OP_CREATE), sc::h160(ParseHex(GENESIS_CONTRACT_ADDRESS_ETH)), ParseHex(GENESIS_CONTRACT_CODE));
...
uint32_t GetNextPhasePlayerNumber()
{
    std::vector<unsigned char> output;
    auto data = CASINO_GETTOTALPLAYER;
...
bool SetNextPhaseWinners(std::string winners, uint32_t currentPhase)
{
    std::vector<unsigned char> output;
    auto data = CASINO_SETNEXTWINNERS + str60zero + ConvertUnsignedIntToHexString(currentPhase) + winners;
    LogPrintf("CASINOMINER ---  set winner opcode data %s \n", data);
    CallContract(sc::h160(ParseHex(GENESIS_CONTRACT_ADDRESS_ETH)), ParseHex(data), &output, true);
    return true;
}

bool CasinoWinners(int seed, uint32_t currentPhase)
{
    //return;
    std::vector<int> winner(CHAIN_PHASE_PLAYER);

    uint32_t totalPlayer = GetNextPhasePlayerNumber();
    LogPrintf("CASINOMINER ---  player number next phase %d \n", totalPlayer);
    ...
    std::string str = str64zero.substr(0, 64 - (4 * CHAIN_PHASE_PLAYER));
    for (uint32_t i = 0; i < CHAIN_PHASE_PLAYER; i++) {
        str += ConvertUnsignedIntToHexString(winner[i] + 1);
    }

    LogPrintf("CASINOMINER ---  winner next phase %s will put into contact, \n\n", str);

    SetNextPhaseWinners(str, currentPhase);

    return true;
}

bool CasinoTime(int nHeight)
{
    ...
    auto currentIndex = (nHeight - 1 ) % CHAIN_PHASE_SIZE;

    if ( currentIndex == CHAIN_PHASE_SIZE - 1) {
        LogPrintf("CASINOMINER ---  generate next phase \n");
        // call contract to find next phase winners
        CasinoWinners(currentPhase, currentPhase);
        return true;
    }
