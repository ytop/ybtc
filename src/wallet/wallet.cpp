...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CYbtcAddress(CScriptID(redeemScript)).ToString();
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFlushOnClose)
{
    ...
    //// debug print
    //jyN LogPrintf("AddToWallet %s  %s%s\n", wtxIn.GetHash().ToString(), (fInsertedNew ? "new" : ""), (fUpdated ? "update" : ""));
                           ...
                           std::list<COutputEntry>& listSent, CAmount& nFee, std::string& strSentAccount, const isminefilter& filter) const
{
    ...
    for (unsigned int i = 0; i < tx->vout.size(); ++i)
    {
        ...
        if ( !txout.scriptPubKey.HasOpCreate() && !txout.scriptPubKey.HasOpCall() && !ExtractDestination(txout.scriptPubKey, address) && !txout.scriptPubKey.IsUnspendable())
        {
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/ybtc/ybtc/pull/10340, but that change has been
    // postponed until after https://github.com/ybtc/ybtc/pull/10244 to
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign, CAmount nGasFee, bool hasSender)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-ybtc-address
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    if (IsDust(txout, ::dustRelayFee) && !hasSender)
                    {
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CYbtcAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CYbtcAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CYbtcAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CYbtcAddress(address).ToString());
 ...
 * https://ybtctalk.org/?topic=54527, or
 * https://github.com/ybtc/ybtc/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CYbtcAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CYbtcAddress(dest).ToString(), key);
}
