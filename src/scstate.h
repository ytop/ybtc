...
#ifndef FABCOIN_SCSTATE_HPP
#define FABCOIN_SCSTATE_HPP
...
struct Change {
    ...
    Change(Kind _kind, Address const& _addr, u256 const& _value = 0) : kind(_kind), address(_addr), value(_value)
    {
    }

    /// Helper constructor especially for storage change log.
    Change(Address const& _addr, u256 const& _key, u256 const& _value) : kind(Storage), address(_addr), value(_value), key(_key)
    {
    }
};
}
...
public:
    ...
    ExecutionResult execute(Transaction const& _t, Permanence _p = Permanence::Committed, OnOpFunc const& _onOp = OnOpFunc());
    ...
    virtual void addBalance(Address const& _id, u256 const& _amount); // fasc
