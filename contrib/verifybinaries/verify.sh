...
###   ybtccore.org and ybtc.org and compares them.
...
WORKINGDIR="/tmp/ybtc_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://ybtccore.org"
HOST2="https://ybtc.org"
BASEDIR="/bin/"
VERSIONPREFIX="ybtc-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ybtc-',
...
if [ $? -ne 0 ]; then
   echo "ybtc.org failed to provide signature file, but ybtccore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "ybtc.org and ybtccore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Ybtc Core binary release signing key installed?"
   fi
