...
The RPM spec file provided here is for Ybtc-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://ybtc.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Ybtc git repository](https://github.com/ybtc/ybtc) but are not part of
...
Currently only `Source10` is used, for the example `ybtc.conf` file.
...
in the ybtc git repository. At present only an SVG version of the Ybtc
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the ybtc
...
needed when 0.12.1 is released, a proper fix is already in the Ybtc git
master branch.
...
idea to build Ybtc Core against a static version of that library compiled
...
The `ybtc.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `ybtc-server` package. At this point, most RPM based
...
The `ybtc-server` package should have SELinux support. How to properly do
...
Ybtc-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/ybtc/). He has been
packaging Ybtc for Fedora at least since 2012.
...
4. I renamed the `ybtc` package that contains the Qt GUI to `ybtc-core` as
that appears to be how the general population refers to it, in contrast to
`ybtc-xt` or `ybtc-classic`. I wanted to make sure the general population
