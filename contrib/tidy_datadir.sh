#!/bin/bash
# Copyright (c) 2013 The Ybtc Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Ybtc database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Ybtc datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Ybtc datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Ybtc 0.7 datadir."
    ;;
  3)
    echo "Detected Ybtc pre-0.8 datadir."
    ;;
  4)
    echo "Detected Ybtc 0.8 datadir."
    ;;
esac
